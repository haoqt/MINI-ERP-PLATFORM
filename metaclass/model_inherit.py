...
class OrderInherit(Order):
    percent = Field(int)
    rate = Field(ftype=int, default=99)

if __name__ == '__main__':
    order = Order()
    print(f"order: {order}, _fields: {order._fields}")
    order_inherit = OrderInherit()
    print(f"order: {order_inherit}, _fields: {order_inherit._fields}")
    print(order_inherit.rate)